...
class _FrontpageState extends State<Frontpage>
 ...
 final GlobalKey <ScaffoldState>  key =GlobalKey();
  ...
  final present = List<bool>.generate(150, (int bool) => true, growable: true);
  @override
                                                                  ...
                                                                  builder: (context,
                                                                    ...
                                                                    return ListView.builder(
                                                                        ...
                                                                        itemBuilder:(BuildContextcontext, index) {
                                                                          return
                                                                            snapshot.data!.docs[index]["admitclass"]==_typeAheadControllerclass.text &&  snapshot.data!.docs[index]["section"]==_typeAheadControllersection.text ?
                                                                            Row(
                                                                              mainAxisAlignment: MainAxisAlignment.spaceAround,
                                                                              children: [
                                                                                Padding(
  ...
  attendaceupload() async {
    ...
    for(int i=0;i<document.docs.length;i++){
      if(document.docs[i]["admitclass"]==_typeAheadControllerclass.text && document.docs[i]["section"]==_typeAheadControllersection.text){
        FirebaseFirestore.instance.collection("Attendance").doc("${_typeAheadControllerclass.text}${_typeAheadControllersection.text}").collection("${DateTime.now().day}-${DateTime.now().month}-${DateTime.now().year}").doc().set({
